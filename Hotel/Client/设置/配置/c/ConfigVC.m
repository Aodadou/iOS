...
- (void)viewDidLoad{
    ...
    NSArray<GroupInfo*> *groupList = [MyGlobalData getGroupInfoList];
    if(groupList.count > 0){
        GroupInfo *groupInfo = [groupList firstObject];
        self.placeName = groupInfo.groupName;
    }else{
        self.placeName = @"暂无房间";
    }
    ...
    UILabel *lb_room = [[UILabel alloc] init];
    lb_room.text = @"请选择房间";
    lb_room.textColor = TFColor;
    ...
    self.img_roomBack = [[UIImageView alloc] init];
    ...
    self.lbRoomName = [[UILabel alloc] init];
    self.lbRoomName.textColor = TFColor;
    self.btn_selectRoom = [[UIButton alloc] init];
    [self.btn_selectRoom addTarget:self action:@selector(actionSelectRoom:) forControlEvents:UIControlEventTouchUpInside];

    [self.view addSubview:lb_ssid];
    [self.view addSubview:lb_pws];
    [self.view addSubview:lb_room];
    [self.view addSubview:self.img_ssidBack];
    [self.view addSubview:self.img_pwdBack];
    [self.view addSubview:self.img_roomBack];
    [self.view addSubview:self.tf_routerPwd];
    [self.view addSubview:self.tf_ssid];
    [self.view addSubview:self.lbRoomName];
    [self.view addSubview:self.btn_selectRoom];
    ...
    self.lbRoomName.text = self.placeName;
    self.img_ssidBack.image = [UIImage imageNamed:@"配置设备输入框"];
    self.img_pwdBack.image = [UIImage imageNamed:@"配置设备输入框"];
    self.wifiImage.image = [UIImage imageNamed:@"WIFI配置"];
    self.img_roomBack.image = [UIImage imageNamed:@"配置设备输入框"];
    [self.btn_showPwd setBackgroundImage:[UIImage imageNamed:@"记住密码按钮"] forState:UIControlStateNormal];
    [self.btn_showPwd setBackgroundImage:[UIImage imageNamed:@"记住密码按钮按下"] forState:UIControlStateHighlighted];
    [self.btn_config setBackgroundImage:[UIImage imageNamed:@"登陆按钮"] forState:UIControlStateNormal];
    [self.btn_config setBackgroundImage:[UIImage imageNamed:@"登陆按钮按下"] forState:UIControlStateHighlighted];
    [self.btn_selectRoom setBackgroundImage:[UIImage imageNamed:@"房间箭头按下"] forState:UIControlStateHighlighted];
    [self.btn_selectRoom setBackgroundImage:[UIImage imageNamed:@"房间箭头"] forState:UIControlStateNormal];
    self.fastLight.image = [UIImage imageNamed:@"快闪灯"];
    ...
    [lb_room mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(lb_ssid.mas_width);
        make.height.equalTo(lb_ssid.mas_height);
        make.left.equalTo(lb_ssid.mas_left);
        make.top.equalTo(_img_pwdBack.mas_bottom).offset(15);
    }];
    [_img_roomBack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(lb_ssid);
        make.height.equalTo(_img_ssidBack);
        make.centerX.equalTo(self.view);
        make.top.equalTo(lb_room.mas_bottom).offset(15);
    }];
    [_lbRoomName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(35);
        make.left.equalTo(_img_roomBack.mas_left).offset(10);
        make.right.equalTo(_btn_selectRoom.mas_left).offset(-10);
        make.centerY.equalTo(_img_roomBack);
    }];
    [_btn_selectRoom mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(_img_roomBack).with.multipliedBy(2.0 / 3);
        make.width.equalTo(_btn_selectRoom.mas_height);
        make.right.equalTo(_img_roomBack.mas_right).offset(-10);
        make.centerY.equalTo(_img_roomBack);
    }];
    ...
    [_btn_config mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(_img_ssidBack);
        make.centerX.equalTo(self.view);
        make.top.equalTo(_img_roomBack.mas_bottom).offset(40);
    }];
...
- (void)actionSelectRoom:(UIButton *)sender{
    ...
    CGFloat viewW = SCREEN_WIDTH * 0.733 - 2;
    CGFloat viewH = viewW * 1.11;
    CGFloat viewX = (SCREEN_WIDTH - viewW) / 2;
    CGFloat viewY = self.img_pwdBack.frame.origin.y;
    if([[UIScreen mainScreen] bounds].size.height == 480){
        viewY = self.img_roomBack.frame.origin.y - 80;
    }
    CGRect viewFrame = CGRectMake(viewX, viewY, viewW , viewH);
    NSString *name;
    if (self.lbRoomName.text){
        name = self.lbRoomName.text;
    }else{
        name = @"";
    }
    
    
    RoomSelectView *roomSelView = [[RoomSelectView alloc] initWithFrame:viewFrame roomName:name btnWidth:((UIButton *)sender).frame.size.width];
    
    roomSelView.blockSelectName = ^(NSString *name,NSString *groupId){
        [backView removeFromSuperview];
        self.lbRoomName.text = name;
        self.groupId = groupId;
    };
    
    [self.view addSubview:roomSelView];
}
...
-(void)receiveCMD:(NSNotification*)obj{
    ...
    if (cmd->CommandNo == [CMD03_ServerLoginRespond commandConst]) {
        ...
        if (cmd03.result) {
            ...
            CMD0C_AddMasterDevice * cmd0c = [[CMD0C_AddMasterDevice alloc] initWithPass:HOTEL_DEFAULT_PWD Mac:macAddress Name:trueDname Place:self.placeName DeviceType:devType];
            [sm sendCmd:cmd0c];
...
- (void)thirdStep{
    ...
    img_iphone.image = [UIImage imageNamed:@"ios多一步图"];
    img_iphone.tag = 505;
